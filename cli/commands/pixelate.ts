...
export function registerPixelateCommand(program: Command) {
  const cmd = program
    ...
    .option("--pixel-size <n>", "source pixels per output pixel", (v) => parseIntArg("pixel-size", v), 4)
    .option("--colors <n>", "palette size (0 = no quantization)", (v) => parseIntArg("colors", v), 16)
    .option(
      "--palette <id>",
      `preset palette id (${PALETTES.map((p) => p.id).join(", ")})`,
      "none",
    )
