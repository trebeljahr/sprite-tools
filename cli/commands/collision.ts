...
export function registerCollisionCommand(program: Command) {
  const cmd = program
    ...
    .option("--alpha <n>", "alpha threshold 0-255 (pixels above are opaque)", (v) =>
      parseFloatArg("alpha", v), 10,
    )
    .option("--tolerance <n>", "RDP simplification tolerance in px (0 = keep every point)", (v) =>
